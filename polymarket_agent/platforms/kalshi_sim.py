...
Simulates a trade whenever the same event shows >2.5% price divergence.
"""
import asyncio
...
MIN_EDGE = 0.025        # 2.5% minimum spread (lowered from 1.5% for more signals)
MAX_POSITION_PCT = 0.10
_SCAN_LOG: deque = deque(maxlen=100)  # detailed scan log for debugging
...
_scanned: int = 0
_scanned_this_session: int = 0
_kalshi_markets_cache: List[dict] = []
_last_scan_ts: Optional[str] = None
_last_scan_detail: dict = {}  # debug info from most recent scan
...
def _match_polymarket(kalshi_title: str, poly_markets: List[dict]) -> Optional[dict]:
    """Find best-matching Polymarket market for a Kalshi event (≥1 keyword match)."""
...
async def scan(poly_markets: List[dict]) -> dict:
    global _balance, _total_pnl, _wins, _scanned, _scanned_this_session
    ...
    _scanned_this_session += len(kalshi_markets)
    if kalshi_markets:
    ...
    for km in kalshi_markets:
        ...
        if not poly_markets:
            detail["reasons_no_trade"].append("no_poly_markets_available")
            break
        ...
        if edge < MIN_EDGE:
            detail["reasons_no_trade"].append(
                f"spread_too_low: {title[:30]} edge={edge:.3f} < {MIN_EDGE}"
            )
            continue
    ...
    print(
        f"[KALSHI] Scan done | fetched={len(kalshi_markets)} "
        f"with_price={prices_found} matched={matched} "
        f"arb_found={detail['spread_above_min']} "
        f"traded={detail['trades_this_scan']} "
